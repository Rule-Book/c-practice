...
void prtreverse(char input[], int length, char output[]);
...
void prtreverse(char input[], int length, char output[])
{
	...
	// printf("max=%d\n", length);
	// int offset = 3;
	// printf("input[(%d)-%d] = %d and that index of input[] is %d\n", length, offset, length-offset, input[length-offset]);
	for (i=0; i<length /*&& input[i]!='\0'*/; ++i) {
		putchar(input[length-1]);
		if (input[length-i] == '\0')
			output[length-i] = '\0';
		else	
